...
There are three ways of installing the application:
...
* By compiling the application yourself.

## Running

If you use Docker, use something as follows:

`docker run -p 5000:5000 --name saxon-server -e JAVA_OPTS="-Xmx1g" -d saxon-server`

Otherwise, start the server by running `java -jar saxon-1.15.jar`.
...
is recommended to place a reverse proxy server in front of this application to take care of IP whitelisting, rate limiting, etc.
